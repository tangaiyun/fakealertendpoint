...
import com.plsseb.alert.bean.integration.een.v2.EENAlertRequest;
...
@WebService(endpointInterface = "com.plsseb.event.notification.v2.INotify", targetNamespace = "http://notification.event.successfactors.com")
public class NotifyImpl implements INotify {
	...
	@Override
	public EENAlertResponsePayload notify(EENAlertRequestMeta externalEventMeta, Events events) {
		System.out.println(">===========HELLO SUCCESSFACTORS V2=============: " + new java.util.Date());
		...
		System.out.println(">===========BYEBYE SUCCESSFACTORS V2=============: " + new java.util.Date());
		return payload;
	}

	private String marshall(EENAlertRequest<EENAlertRequestMeta, EENAlertRequestData> request) {
		try {
			Class<?>[] dynamicBoundTypes = request.dynamicBoundClasses();
			Class<?>[] jaxbTypes = Arrays.copyOf(dynamicBoundTypes, dynamicBoundTypes.length + 1);
